    ...
    <div className="h-screen w-full flex">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center bg-white px-8">
        <div className="w-full max-w-md">
          ...
          <p className="text-gray-500 text-center mt-2 mb-8">Enter your email and verification method</p>
          <form onSubmit={isResetStage ? resetSubmit : requestReset} className="space-y-5 w-full">
            <div className="flex flex-col">
              ...
              required
              disabled={emailFixed}
              className={`text-black px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none ${emailFixed ? "bg-gray-200 text-gray-500" : ""}`}
            />
            </div>

            <div className="flex flex-col">
              <label className="text-sm text-gray-600 mb-1">Verification method</label>
              <div className="flex space-x-4">
                {(["key", "question"] as const).map((method) => (
                  <label key={method} className="inline-flex items-center">
                    <input type="radio" name="method" value={method} checked={form.method === method}
                      onChange={() => setForm((f) => ({ ...f, method }))} className="form-radio" />
                    <span className="ml-2 text-gray-800">{method === "key" ? "Reset key" : "Security question"}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            {form.method === "key" ? (
              <div className="flex flex-col">
                ...
                <input type="text" value={form.key} onChange={(e) => setForm((f) => ({ ...f, key: e.target.value }))}
                  placeholder="Enter reset key" className="text-black px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
              </div>
            ) : (
              <div className="space-y-3">
                <div className="flex flex-col">
                  <label className="text-sm text-gray-600 mb-1">Security question</label>
                  {status.loading ? <p className="text-gray-700">Loading your question...</p> : form.question ? <p className="text-black px-4 py-3 border border-gray-300 rounded-lg bg-gray-50">{form.question}</p> : <p className="text-red-500">{status.qError || "No security question available."}</p>}
                </div>
                <div className="flex flex-col">
                  ...
                  <input type="text" value={form.answer} onChange={(e) => setForm((f) => ({ ...f, answer: e.target.value }))}
                    placeholder="Enter your security answer" className="text-black px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                </div>
              </div>
            )}
              ...
              <>
                <div className="flex flex-col">
                  ...
                  <input type="text" placeholder="Enter token" value={form.token} onChange={(e) => setForm((f) => ({ ...f, token: e.target.value }))}
                    required className="text-black px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                </div>
            ...
            {status.validation && <p className="text-red-500 text-sm mt-1">{status.validation}</p>}
            {status.localError && <p className="text-red-500 text-sm mt-1">{status.localError}</p>}
          ...
          <p className="text-md text-center mt-4"><span className="text-blue-600 hover:underline cursor-pointer text-md" onClick={() => navigate(-1)}>Back </span></p>
        </div>
      </div>
